...
-   `R` (folder): Contains .R files of scripts and subworkflows. Workflows do not go into a folder.
    -   `0_init.R`: Loads packages, solves conflicts with package `conflicted`, loads options, and initialises the cache.
-   `Stan` (folder): Contains .stan files. (See Options)
-   `report.qmd`: A quarto file containing the basic project structure: "Pre-data" describes tasks prior to using data, especially simulation, "Data" describes processing of data, "Post-data" describes data analysis.
...
Create a new script with function `new_file()`, which is also accessible via a RStudio Addin. Simply follow the instructions in the dialog boxes.
